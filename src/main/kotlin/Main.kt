...
// Import model classes from extracted packages
...
fun main() {
    ...
    for (file in kotlinFiles) {
        ...
        for (classOrObject in ktFile.declarations.filterIsInstance<KtClassOrObject>()) {
            val analysis = analyzeClass(classOrObject, file.name, allKtFiles.flatMap { it.declarations.filterIsInstance<KtClassOrObject>() })
            analyses.add(analysis)
        }
    }
    
    // Process Java files
    val allJavaClasses = mutableListOf<ClassOrInterfaceDeclaration>()
    
    // First pass: collect all Java classes
    for (file in javaFiles) {
        try {
            val cu = StaticJavaParser.parse(file)
            allJavaClasses.addAll(cu.findAll(ClassOrInterfaceDeclaration::class.java))
        } catch (e: Exception) {
    ...
    for (file in javaFiles) {
        try {
            ...
            cu.findAll(ClassOrInterfaceDeclaration::class.java).forEach { classDecl ->
                val analysis = analyzeJavaClass(classDecl, file.name, allJavaClasses)
                analyses.add(analysis)
            }
        } catch (e: Exception) {
            println("Error parsing Java file ${file.name}: ${e.message}")
    ...
    for (ktFile in ktFiles) {
        ...
        for (classOrObject in ktFile.declarations.filterIsInstance<KtClassOrObject>()) {
            ...
            val nodeType = when {
                ...
                classOrObject is KtClass && classOrObject.isAbstract() -> NodeType.CLASS
        ...
        try {
            ...
            cu.findAll(ClassOrInterfaceDeclaration::class.java).forEach { classDecl ->
                ...
                val nodeType = when {
                    ...
                    classDecl.isAbstract -> NodeType.CLASS
                    else -> NodeType.CLASS
                }
    ...
    fun dfs(nodeId: String, path: MutableList<String>): Boolean {
        if (recursionStack.contains(nodeId)) {
            ...
            if (cycleStart != -1) {
                ...
                val severity = when {
                    cyclePath.size <= 3 -> "LOW"
                    cyclePath.size <= 6 -> "MEDIUM"
                    else -> "HIGH"
                }
...
fun analyzeDomainEvent(classOrObject: KtClassOrObject, className: String, fileName: String): DddDomainEvent {
    ...
    return DddDomainEvent(
        ...
        fileName = fileName,
        isEvent = isEvent,
        isImmutable = false, // Simplified for now
        hasEventNaming = className.endsWith("Event"),
        hasTimestamp = false, // Simplified for now
        confidence = confidence.coerceAtMost(1.0)
    )
}
...
fun analyzeClass(classOrObject: KtClassOrObject, fileName: String, allKtClasses: List<KtClassOrObject> = emptyList()): ClassAnalysis {
    val className = classOrObject.name ?: "Anonymous"
...
fun analyzeJavaClass(classDecl: ClassOrInterfaceDeclaration, fileName: String, allJavaClasses: List<ClassOrInterfaceDeclaration> = emptyList()): ClassAnalysis {
    val className = classDecl.nameAsString
